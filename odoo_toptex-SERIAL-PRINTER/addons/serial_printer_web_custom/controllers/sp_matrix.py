...
class SpMatrix(http.Controller):
    ...
    def sp_matrix_combos(self, template_id, **kw):
        """Devuelve todas las variantes del template con:
           - product_id
           - ptav_ids / pav_ids (para hacer match color/talla)
           - price (website_price) y stock (qty_available)
           - image (url /web/image/...)
        """
        ...
        for v in template.product_variant_ids.sudo():
            ...
            # Precio visible en web (respeta la pricelist activa). Fallback a lst_price.
