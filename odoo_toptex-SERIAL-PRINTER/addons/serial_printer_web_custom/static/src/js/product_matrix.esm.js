...
(function () {
  ...
  async function jsonrpc(url, params) {
    // type='json' en Odoo espera JSON-RPC
  ...
  async function fetchCombination(ptavIds, root) {
    ...
    // Intentamos los endpoints web en orden (versiones/ediciones distintas):
    const endpoints = [
    ...
    for (const url of endpoints) {
      try {
        ...
        // Si el endpoint existe pero devuelve Odoo Server Error, seguimos probando el siguiente
        warn('combination por', url, 'falló:', e?.rpc || e?.message || e);
        continue;
      }
    }

    // Si nada respondió, devolvemos null
    return null;
  }
