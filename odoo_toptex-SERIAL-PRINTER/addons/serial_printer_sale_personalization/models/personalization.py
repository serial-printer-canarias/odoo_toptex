...
class SpPersonalizationOrder(models.Model):
    ...
    def _groups_need_refresh(self):
        self.ensure_one()
        if self.marking_count:
            return False
        for group in self.group_ids:
            if "_" in (group.product_ref or "") or len(group.product_name or "") > 58:
                return True
        return False
    ...
    def _create_group_records(self, grouped, line_model):
        for data in grouped.values():
            size_summary = " · ".join("%s: %s" % (size, _format_qty(qty)) for size, qty in sorted(data["sizes"].items(), key=_size_sort_key))
            vals = {
