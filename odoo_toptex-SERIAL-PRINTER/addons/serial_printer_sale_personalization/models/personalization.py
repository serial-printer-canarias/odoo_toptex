...
class SpPersonalizationOrder(models.Model):
    ...
    def _groups_need_refresh(self):
        ...
        for group in self.group_ids:
            ref = _model_code_from_values(group.product_ref, group.product_name, group.product_id)
            key = (ref or _norm(group.product_name), _norm(group.color_name))
            if key in seen:
                return True
            seen.add(key)
    ...
    def _normalize_and_merge_groups(self):
        for rec in self:
            ...
            for group in rec.group_ids.sorted("id"):
                ...
                key = (ref or _norm(short_name), _norm(color))
                if key in buckets:
    ...
    def _group_line(self, grouped, product, qty, source_line=False):
        ...
        key = (product_ref or _norm(product_name) or (product.product_tmpl_id.id if product else ""), color)
