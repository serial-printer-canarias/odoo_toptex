...
class SpPersonalizationPlacement(models.Model):
    ...
    @api.depends("preview_image", "preview_attachment_id.mimetype", "production_attachment_id.mimetype")
    def _compute_logo_preview_available(self):
        for rec in self:
            rec.logo_preview_available = bool(
                rec.preview_image
                or _attachment_is_image(rec.preview_attachment_id)
                or _attachment_is_image(rec.production_attachment_id)
            )
