...
class SerialPrinterPersonalizationPortal(http.Controller):
    ...
    def _save_marking_from_post(self, personalization, post):
        ...
        # Releer ubicaciones tras guardar posición, porque write() puede haber sincronizado
        # pecho/espalda/cuello y el PDF debe leer exactamente estas ubicaciones actualizadas.
        marking.invalidate_recordset(["placement_ids"])
    ...
    def personalization_form(self, access_token, **post):
        ...
        if request.httprequest.method == "POST":
            ...
            if action == "add_marking":
                ...
                self._save_marking_from_post(personalization, post)
            elif action == "confirm":
                # La confirmación solo cambia el estado. Los marcajes se guardan antes desde el
                # formulario cliente mediante las mismas llamadas save_marking para no perder
                # técnica, color, tallas, archivos ni previews antes de generar el PDF taller.
