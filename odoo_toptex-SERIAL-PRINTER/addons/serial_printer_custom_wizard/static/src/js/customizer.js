/* Previsualización del logo sobre la imagen del producto.
   No toca nada fuera del customizer (actúa sólo si existe #spw_canvas). */
(function () {
  'use strict';
  ...
  function init() {
    ...
    // Reset público (el botón Reset existente lo llama)
    window.spwReset = function () {
      ...
      state.widthPct = 100; state.dxPct = 0; state.dyPct = 10; state.rotDeg = 0;
      applyTransform();
    };
    ...
    // Subida de logo (siempre FileReader para mayor compatibilidad)
    if (logoInput && logoImg) {
      logoInput.addEventListener('change', (ev) => {
        ...
        reader.onload = (e) => {
          ...
          applyTransform();
          // subir viewport a la imagen en móvil
