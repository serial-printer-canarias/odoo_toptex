...
class SPWCartMeta(http.Controller):
    ...
    def add_to_cart_meta(self, variant_id, qty=1, tech=None, svg_color=None, notes=None, spw_token=None, **kw):
        ...
        # NO pasar line_id; pasamos el spw_token para forzar línea nueva
        ...
        # Texto de la línea SOLO de esta personalización (no acumular anteriores)
        meta = []
    ...
    def attach_png(self, line_id, png_b64, **kw):
        """Una preview por línea; cada línea tiene su propia imagen, no se pisan entre líneas."""
        Attach = request.env['ir.attachment'].sudo()
        name = 'spw_preview_%s.png' % int(line_id)
        vals = {
            'name': name, 'res_model': 'sale.order.line', 'res_id': int(line_id),
            'type': 'binary', 'mimetype': 'image/png', 'datas': png_b64,
        }
        existing = Attach.search([('res_model','=','sale.order.line'), ('res_id','=',int(line_id)), ('name','=',name)], limit=1)
        (existing and existing.write(vals)) or Attach.create(vals)
        return {'ok': True}