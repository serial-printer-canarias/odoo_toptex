...
# PNG transparente 1x1 para fallback (evita 404)
...
class SpwReport(http.Controller):
    ...
    def spw_line_preview(self, line_id, **kw):
        """Devuelve el PNG de personalización de una línea.
        Nunca 404: si no hay, responde un 1x1 transparente."""
        env = request.env

        # Soporta sale.order.line y account.move.line
        ...
        # 1) Buscar adjunto de imagen asociado a la línea
        ...
        # 2) Fallback: posible campo binario en la línea (si existiera)
        ...
        # 3) Último recurso: 1x1 transparente (evita 404/500)
        return request.make_response(BLANK_PNG, headers=[('Content-Type', 'image/png')])