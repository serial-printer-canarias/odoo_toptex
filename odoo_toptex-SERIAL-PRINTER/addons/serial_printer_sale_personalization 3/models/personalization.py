...
PRINT_COLOR_SELECTION = [
    ...
    ("water_green", "Verde agua"), ("mint", "Menta"),
    ("soft_mint", "Menta suave"), ("sage", "Salvia"),
    ("eucalyptus", "Eucalipto"), ("pastel_aqua", "Aqua pastel"),
    ("baby_blue", "Azul bebé"), ("powder_blue", "Azul polvo"),
    ("pastel_blue", "Azul pastel"), ("lavender", "Lavanda"),
    ("pastel_lilac", "Lila pastel"), ("mauve", "Malva"),
    ("soft_purple", "Morado suave"), ("pearl", "Perla"),
    ("sand", "Arena"), ("light_beige", "Beige claro"),
    ("pink_beige", "Beige rosado"), ("soft_taupe", "Taupe suave"),
    ("light_camel", "Camel claro"),
]
...
TECHNIQUE_SELECTION = [
    ...
    ("embroidery", "Bordado"),
    ("dtf", "DTF"),
    ("to_advise", "A recomendar"),
]

POSITION_SELECTION = [
    ("front_left", "Pecho izquierdo"),
    ("front_center", "Pecho centro"),
    ("back", "Espalda"),
    ("front_back", "Pecho + espalda"),
    ("neck", "Cuello / etiqueta"),
    ("front_back_neck", "Pecho + espalda + cuello"),
    ("right_sleeve", "Manga derecha"),
    ("left_sleeve", "Manga izquierda"),
    ("cap_front", "Frontal gorra"),
    ("cap_side", "Lateral gorra"),
    ("cap_front_side", "Frontal + lateral gorra"),
]

SIZE_MODE_SELECTION = [
    ("proportional", "A proporción"),
    ("small", "Pequeño"),
    ("medium", "Medio"),
    ("large", "Grande"),
    ("exact", "Medida exacta"),
...
MODEL_REQUIRED_PLACEMENTS = {
    ...
    "front_back_neck": [
        ("front_left", "Pecho", "front"),
        ("back", "Espalda", "back"),
        ("neck", "Cuello / etiqueta", "back"),
    ],
...
def _selection_label(record, field_name, value):
    return dict(record._fields[field_name].selection).get(value or "", value or "")
...
class SpPersonalizationOrder(models.Model):
    ...
    state = fields.Selection([
        ("draft", "Borrador"), ("sent", "Enviado al cliente"), ("customer_done", "Cliente completó"),
        ("reviewed", "Revisado"), ("production", "En producción"), ("done", "Finalizado"), ("cancel", "Cancelado"),
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if vals.get("name", "Nueva") == "Nueva":
                vals["name"] = seq.next_by_code("sp.personalization.order") or "Nueva"
    ...
    def action_notify_received(self):
        ...
        if template:
            template.send_mail(self.id, force_send=True)
        self.message_post(body=_("El cliente ha enviado/completado la ficha de personalización."))
        return True
    ...
    def _create_groups_from_sale_order(self, sale_order):
        ...
        self._create_groups_from_grouped_data(grouped)
    ...
    def _create_groups_from_grouped_data(self, grouped):
        self.ensure_one()
        for data in grouped.values():
            size_summary = " · ".join(
                "%s: %s" % (size, int(qty) if float(qty).is_integer() else qty)
            ...
            self.env["sp.personalization.group"].create({
                ...
                "sale_line_ids": [(6, 0, data.get("sale_line_ids", []))],
            })
...
class SpPersonalizationGroup(models.Model):
    ...
    state = fields.Selection(related="personalization_id.state", store=True)

    def action_add_marking(self):
        for group in self:
            position = "front_left" if group._is_textile() else "cap_front"
            technique = "screen" if group._is_textile() else "embroidery"
    ...
    def _is_textile(self):
        self.ensure_one()
        name = _safe_lower(self.product_name)
        ref = _safe_lower(self.product_ref)
        return not ("gorra" in name or "cap" in name or ref.startswith("kp111"))
...
class SpPersonalizationMarking(models.Model):
    ...
    technique_label = fields.Char(compute="_compute_labels", store=True)
    position = fields.Selection(POSITION_SELECTION, string="Posición", default="front_left", required=True)
    position_label = fields.Char(compute="_compute_labels", store=True)
    print_color = fields.Selection(PRINT_COLOR_SELECTION, string="Color tinta/hilo", default="white")
    print_color_label = fields.Char(compute="_compute_labels", store=True)
    ...
    size_mode_label = fields.Char(compute="_compute_labels", store=True)
    ...
    @api.depends("technique", "position", "print_color", "size_mode")
    def _compute_labels(self):
        for rec in self:
            rec.technique_label = _selection_label(rec, "technique", rec.technique)
            rec.position_label = _selection_label(rec, "position", rec.position)
            rec.print_color_label = _selection_label(rec, "print_color", rec.print_color)
            rec.size_mode_label = _selection_label(rec, "size_mode", rec.size_mode)
    ...
    def _create_default_size_lines_from_group(self):
        for rec in self:
            if rec.size_line_ids:
                continue
            size_parts = (rec.group_id.size_summary or "").split("·")
            for part in size_parts:
                ...
                try:
                    ...
                    qty_float = 0.0
                self.env["sp.personalization.marking.size"].create({
                    "marking_id": rec.id,
    ...
    def _get_required_placements(self):
        self.ensure_one()
        return MODEL_REQUIRED_PLACEMENTS.get(self.position, MODEL_REQUIRED_PLACEMENTS["front_left"])

    def _sync_placements(self):
        for rec in self:
            required = rec._get_required_placements()
            required_keys = [x[0] for x in required]
            ...
            for key, label, side in required:
                if key not in existing:
                    self.env["sp.personalization.placement"].create({
                        "marking_id": rec.id,
                        "placement_key": key,
...
class SpPersonalizationPlacement(models.Model):
    ...
    def _create_attachment_from_upload(self, upload_file, field):
        self.ensure_one()
        if not upload_file:
            return False
        ...
        if field == "preview_attachment_id" or ext in ["png", "jpg", "jpeg", "webp", "svg"]:
