...
def get_token():
    icp = models.env['ir.config_parameter'].sudo()
...
class ProductTemplate(models.Model):
    ...
    def sync_product_from_api(self):
        ...
        # --- PRECIO COSTE Y VENTA POR VARIANTE ---
        # (Sólo precio, stock y fotos por variante se hace después)
        try:
            ...
            price_resp = requests.get(price_url, headers=headers_price)
    ...
    # ------ SERVER ACTION PARA FOTOS POR VARIANTE ------
    def sync_images_by_variant(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                data = data_list if isinstance(data_list, dict) else data_list[0] if data_list else {}
                colors = data.get("colors", [])
                # Mapeo Odoo <-> TopTex por nombre color
    ...
    # ------ SERVER ACTION PARA STOCK POR VARIANTE ------
        ...
        try:
            ...
            if inv_resp.status_code == 200:
                ...
                for variant in self.product_variant_ids:
                    ...
                    _logger.info(f"📦 Stock actualizado para {variant.name}: {stock}")
        except Exception as e:
