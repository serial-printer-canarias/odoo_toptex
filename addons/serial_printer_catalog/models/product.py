...
class ProductTemplate(models.Model):
    ...
    def sync_product_from_api(self):
        ...
        if not all([api_key, username, password, proxy_url]):
            _logger.error('❌ Faltan parámetros API TopTex')
        ...
        if not token:
            _logger.error('❌ No se recibió token TopTex')
            raise UserError('No se recibió token TopTex')
        headers['x-toptex-authorization'] = token

        # Llamada a TODO el catálogo
        ...
        if not file_link:
            _logger.error('❌ No se obtuvo el link del JSON de productos')
            raise UserError('No se obtuvo el link del JSON de productos')
        json_resp = requests.get(file_link, headers=headers)
        if json_resp.status_code != 200:
            _logger.error(f"❌ Error descargando el JSON catálogo: {json_resp.text}")
        ...
        _logger.info(f"📦 Total productos recibidos: {len(catalog)}")
        ...
        for prod in catalog:
            try:
                ...
                _logger.error(f"❌ Error procesando producto {prod.get('catalogReference', '')}: {e}")
