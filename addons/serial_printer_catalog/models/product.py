...
class ProductTemplate(models.Model):
    ...
    def sync_product_from_api(self):
        ...
        if not all([username, password, api_key, proxy_url]):
            raise UserError("❌ Faltan credenciales o parámetros en el sistema.")
        ...
        list_price = 9.8  # fijo de momento
        standard_price = 0.0

        # Sacar precio de coste (primer size encontrado)
        ...
        for variant in product_template.product_variant_ids:
            color_value = variant.product_template_attribute_value_ids.filtered(lambda v: v.attribute_id.name == 'Color').name
    ...
    def get_image_binary_from_url(self, url):
        try:
            ...
            if response.status_code == 200 and response.headers.get('Content-Type', '').startswith('image'):
                ...
                _logger.warning(f"⚠️ Contenido no válido como imagen desde: {url}")
        except Exception as e:
