...
class ProductTemplate(models.Model):
    ...
    def sync_product_from_api(self):
        ...
        try:
            ...
            if not token:
                raise UserError("❌ No se recibió el token de autenticación.")
        except Exception as e:
            raise UserError(f"❌ Error inesperado al autenticar: {str(e)}")

        # Llamada a productos (NS300)
        ...
        try:
            ...
            self.env['product.template'].create({
                'name': product_data.get('catalogReference', 'Producto NS300'),
                'sale_ok': True,
                'purchase_ok': True,
            })
