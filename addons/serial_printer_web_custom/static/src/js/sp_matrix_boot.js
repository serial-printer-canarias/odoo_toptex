// Ejecuta sin depender del module-loader de Odoo
(function () {
  'use strict';

  // CSS mínimo inline (evitamos SCSS de momento)
  ...
  function findAttributeBlocks(root){
    ...
    for (const el of nodes){
      ...
      let name = el.getAttribute('data-attribute_name') ||
  ...
  ready(function () {
    ...
    if (!page){ console.log('[SP] matrix: no product page'); return; }

    const blocks = findAttributeBlocks(page);
    console.log('[SP] bloques =>', blocks.map(b => ({name:b.name, n:b.options.length})));
    if (blocks.length < 2){ console.log('[SP] faltan atributos color/talla'); return; }
    ...
    // Inserción: justo detrás del último bloque de atributos
