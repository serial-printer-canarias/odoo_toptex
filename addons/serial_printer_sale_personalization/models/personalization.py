...
def _normalized_color_key(value):
    text = _ascii_key(value)
    text = re.sub(r"[/_-]+", " ", text)
    text = re.sub(r"\b(washed|lavado|organic|organico|org)\b", " ", text)
    text = re.sub(r"\b(navy blue|azul marino|marino)\b", "navy", text)
    text = re.sub(r"\b(french red)\b", "red", text)
    text = re.sub(r"\b(reflex blue)\b", "blue", text)
    return re.sub(r"\s+", " ", text).strip() or "sin color"
...
def _model_code_from_values(ref=False, name=False, product=False):
    candidates = [
        ref,
        product.default_code if product else "",
        product.product_tmpl_id.default_code if product else "",
        name,
    ]
    for candidate in candidates:
        ...
        token = text.split("_", 1)[0].split(" ", 1)[0]
        match = re.match(r"^([A-Z]{1,5}\d{2,5}[A-Z]?)", token)
        if match:
            return match.group(1)
    return _short_ref(ref or "")


def _short_product_name(value, model_code=False):
    text = _clean_text(value).replace(" Descargar PNG", "")
    text = re.sub(r"^\[[^\]]+\]\s*", "", text)
    text = re.sub(r"\s*\([^()]*(?:,?\s*(?:XXS|XS|S|M|L|XL|XXL|XXXL|2XL|3XL|4XL|5XL))\)\s*$", "", text, flags=re.I)
    text = re.sub(r"\s+", " ", text).strip()
    if model_code and not text.upper().startswith(model_code.upper()):
        text = "%s %s" % (model_code, text)
...
def _size_sort_key(item):
    size = _norm(item[0])
    return (SIZE_ORDER.get(size, 100), size)


def _format_size_summary(size_map):
    return " · ".join(
        "%s: %s" % (size, _format_qty(qty))
        for size, qty in sorted(size_map.items(), key=_size_sort_key)
    )


def _attachment_is_image(attachment):
    return bool(attachment and attachment.mimetype and attachment.mimetype.startswith("image/"))


def _image_mimetype_from_content(content, fallback="image/png"):
...
def _data_uri_from_b64(value, mimetype=False):
    b64_value = _b64_text(value)
    if not b64_value:
        return ""
    detected_mimetype = mimetype
    if not detected_mimetype or detected_mimetype == "application/octet-stream":
        try:
            detected_mimetype = _image_mimetype_from_content(base64.b64decode(b64_value))
        except Exception:
            detected_mimetype = "image/png"
    return "data:%s;base64,%s" % (detected_mimetype or "image/png", b64_value)


def _png_data_uri_from_b64(value, max_size=(420, 420)):
    b64_value = _b64_text(value)
    if not b64_value:
        return ""
    try:
        ...
        return ""
    if content.lstrip().startswith(b"<svg"):
        return _data_uri_from_b64(b64_value, "image/svg+xml")
    if not Image:
        return _data_uri_from_b64(b64_value)
    try:
...
def _preview_b64_from_content(content, max_size=(1024, 1024)):
    if not content or content.lstrip().startswith(b"<svg") or not Image:
        return False
    try:
        image = Image.open(BytesIO(content))
        if image.mode not in ("RGB", "RGBA"):
...
def _image_from_b64(value, max_size=False):
    if not Image:
        return False
    b64_value = _b64_text(value)
    if not b64_value:
        return False
    try:
        image = Image.open(BytesIO(base64.b64decode(b64_value))).convert("RGBA")
        if max_size:
            image.thumbnail(max_size, Image.LANCZOS)
        return image
    except Exception:
...
def _png_data_uri_from_image(image):
    if not image:
        return ""
    output = BytesIO()
    image.save(output, format="PNG")
...
def _hex_to_rgba(value, default=(31, 60, 255, 255)):
...
def _group_key(product_ref=False, product_name=False, product=False, color=False):
    ref = _model_code_from_values(product_ref, product_name, product)
    model_key = ref or (str(product.product_tmpl_id.id) if product and product.product_tmpl_id else _ascii_key(product_name))
    return (model_key, _normalized_color_key(color))
...
class SpPersonalizationOrder(models.Model):
    ...
    workshop_pdf_attachment_id = fields.Many2one("ir.attachment", string="PDF taller guardado", copy=False)
    workshop_pdf_filename = fields.Char(related="workshop_pdf_attachment_id.name", string="Archivo PDF taller")
    ...
    def action_send_to_customer(self):
        for rec in self:
            if not rec.partner_id.email:
                raise UserError(_("El cliente no tiene email. Añade un email al contacto para enviar el formulario."))
            rec.state = "sent"
            body = _(
                "<p>Hola %s,</p>"
                "<p>Puedes completar la ficha de personalización textil en este enlace:</p>"
                "<p><a href=\"%s\">Abrir formulario de personalización</a></p>"
                "<p>Gracias.</p>"
            ) % (rec.partner_id.name or "", rec.portal_url)
            mail = self.env["mail.mail"].sudo().create({
                "subject": _("Ficha de personalización %s") % (rec.source_display_name or rec.name),
                "body_html": body,
                "email_to": rec.partner_id.email,
                "auto_delete": False,
            })
            mail.send()
            rec.message_post(body=_("Email de personalización enviado al cliente: %s") % rec.portal_url)
        return True
    ...
    def _create_workshop_pdf_attachment(self):
        self.ensure_one()
        report = self.env.ref("serial_printer_sale_personalization.action_report_sp_personalization", raise_if_not_found=False)
        if not report:
            raise UserError(_("No se ha encontrado el informe PDF de taller."))
        pdf_content, _content_type = self.env["ir.actions.report"].sudo()._render_qweb_pdf(report.report_name, [self.id])
        attachment = self.env["ir.attachment"].sudo().create({
            "name": "Ficha taller - %s.pdf" % (self.name or "personalizacion"),
            "type": "binary",
        ...
        self.workshop_pdf_attachment_id = attachment.id
        return attachment

    def action_download_workshop_pdf(self):
        self.ensure_one()
        attachment = self.workshop_pdf_attachment_id or self._create_workshop_pdf_attachment()
        if not attachment:
            raise UserError(_("Todavía no hay PDF de taller."))
        return {
    ...
    def action_download_all_files(self):
        self.ensure_one()
        placements = self.group_ids.mapped("marking_ids").mapped("placement_ids")
        attachments = (
            placements.mapped("production_attachment_id")
            | placements.mapped("preview_attachment_id")
        ).filtered(lambda attachment: attachment and attachment.datas)
        if not attachments:
            raise UserError(_("Todavía no hay archivos subidos por el cliente."))
        output = BytesIO()
        used_names = set()
        with zipfile.ZipFile(output, "w", zipfile.ZIP_DEFLATED) as archive:
            for attachment in attachments:
                base_name = attachment.name or "archivo"
                filename = base_name
                counter = 2
                while filename in used_names:
                    if "." in base_name:
                        stem, ext = base_name.rsplit(".", 1)
                        filename = "%s-%s.%s" % (stem, counter, ext)
                    else:
                        filename = "%s-%s" % (base_name, counter)
                    counter += 1
                used_names.add(filename)
                archive.writestr(filename, base64.b64decode(attachment.datas))
        zip_attachment = self.env["ir.attachment"].sudo().create({
            "name": "%s-archivos-personalizacion.zip" % (self.name or "personalizacion"),
            "type": "binary",
    ...
    def _send_internal_submit_email(self, user, pdf_attachment=False):
        self.ensure_one()
        if not user or not user.email:
            return False
        base_url = self.env["ir.config_parameter"].sudo().get_param("web.base.url", "").rstrip("/")
        backend_url = "%s/web#id=%s&model=sp.personalization.order&view_type=form" % (base_url, self.id)
        body = _(
            "<p>El cliente ha enviado y aceptado la ficha de personalización.</p>"
            "<p><b>Ficha:</b> %s<br/>"
            "<b>Cliente:</b> %s<br/>"
            "<b>Origen:</b> %s</p>"
            "<p><a href=\"%s\">Abrir ficha en Odoo</a></p>"
            "<p><a href=\"%s\">Abrir formulario del cliente</a></p>"
        ) % (
            self.name,
            self.partner_id.display_name or "-",
            self.source_display_name or "-",
            backend_url,
            self.portal_url,
        )
        attachment_ids = pdf_attachment.ids if pdf_attachment else []
        mail_values = {
            "subject": _("Personalización enviada: %s") % (self.source_display_name or self.name),
            "body_html": body,
            "email_to": user.email,
            "auto_delete": False,
        }
    ...
    def action_customer_submit(self, customer_notes=False):
        todo_type = self.env.ref("mail.mail_activity_data_todo", raise_if_not_found=False)
        for rec in self:
            if customer_notes is not False:
                rec.customer_notes = customer_notes or rec.customer_notes
            rec.write({
                "state": "customer_done",
                "confirmed_date": fields.Datetime.now(),
            })
            placements = rec.group_ids.mapped("marking_ids").mapped("placement_ids")
            attachments = (
                placements.mapped("production_attachment_id")
                | placements.mapped("preview_attachment_id")
            ).filtered(lambda attachment: attachment)
            pdf_attachment = self.env["ir.attachment"]
            try:
                pdf_attachment = rec._create_workshop_pdf_attachment()
            except Exception as error:
                rec.message_post(body=_("No se pudo crear automáticamente el PDF de taller: %s") % error)
            notification_attachments = attachments | pdf_attachment
            source_user = rec.sale_order_id.user_id or rec.account_move_id.invoice_user_id or rec.create_uid
            partner_ids = source_user.partner_id.ids if source_user and source_user.partner_id else []
            body = _(
                "El cliente ha enviado la ficha de personalización.<br/>"
                "<b>Ficha:</b> %s<br/>"
                "<b>Origen:</b> %s<br/>"
                "Los archivos originales quedan adjuntos aquí y en cada ubicación de la ficha. El PDF de taller queda guardado en la ficha."
            ) % (rec.name, rec.source_display_name or "-")
            rec.message_post(
                body=body,
                attachment_ids=notification_attachments.ids,
            ...
            rec._send_internal_submit_email(source_user, pdf_attachment)
            if todo_type and source_user:
                rec.activity_schedule(
                    "mail.mail_activity_data_todo",
                    user_id=source_user.id,
                    summary=_("Revisar personalización enviada"),
                    note=_("El cliente ha enviado la ficha %s. Revisa el PDF taller y los archivos adjuntos.") % rec.name,
                )
        return True
    ...
    def _ensure_groups_from_source(self):
        for rec in self:
            if rec.group_ids:
                rec._normalize_and_merge_groups()
            if rec.group_ids and not rec._groups_need_refresh():
                continue
    ...
    def _groups_need_refresh(self):
        self.ensure_one()
        seen = set()
        for group in self.group_ids:
            key = _group_key(group.product_ref, group.product_name, group.product_id, group.color_name)
            if key in seen:
                return True
            seen.add(key)
            if "_" in (group.product_ref or "") or len(group.product_name or "") > 48:
                return True
        return False
    ...
    def _normalize_and_merge_groups(self):
        for rec in self:
            buckets = {}
            for group in rec.group_ids.sorted("id"):
                product = group.product_id or self.env["product.product"].search(
                    [("product_tmpl_id", "=", group.product_tmpl_id.id)],
                    limit=1,
                )
                ref = _model_code_from_values(group.product_ref, group.product_name, product)
                short_name = _short_product_name(group.product_name or (product.product_tmpl_id.display_name if product else ""), ref)
                color = group.color_name or "Sin color"
                key = _group_key(ref, short_name, product, color)
                if key in buckets:
                    buckets[key]._absorb_duplicate_group(group)
                    continue
                buckets[key] = group
                vals = {}
                if ref and group.product_ref != ref:
                    vals["product_ref"] = ref
                if short_name and group.product_name != short_name:
                    vals["product_name"] = short_name
                if not group.product_image_1920 and product:
                    image = product.image_1920 or product.product_tmpl_id.image_1920
                    if image:
                        vals["product_image_1920"] = image
                if vals:
                    group.write(vals)
    ...
    def _line_summary(self, line, product, color, size, qty):
        ...
        ref = _model_code_from_values(parsed["product_ref"] or ref, parsed["product_name"], product)
        details = " / ".join(item for item in [ref, color if color != "Sin color" else "", size if size != "Sin talla" else ""] if item)
        return "%s - %s uds" % (details or _("Línea"), _format_qty(qty))
    ...
    def _group_line(self, grouped, product, qty, source_line=False):
        ...
        raw_product_name = parsed["product_name"] or (product.product_tmpl_id.display_name if product else "Producto sin nombre")
        product_ref = _model_code_from_values(parsed["product_ref"], raw_product_name, product)
        product_name = _short_product_name(raw_product_name, product_ref)
        key = _group_key(product_ref, product_name, product, color)
    ...
    def _create_group_records(self, grouped, line_model):
        for data in grouped.values():
            size_summary = _format_size_summary(data["sizes"])
            vals = {
...
class SpPersonalizationGroup(models.Model):
    ...
    qty_total_display = fields.Char(compute="_compute_qty_total_display")
    size_summary = fields.Char()
    original_line_summary = fields.Text()
    product_image_1920 = fields.Image(max_width=1920, max_height=1920)
    product_image_data_uri = fields.Text(compute="_compute_product_image_data_uri")
    ...
    garment_color_hex = fields.Char(compute="_compute_garment_color_hex")
    ...
    def _compute_qty_total_display(self):
        for group in self:
            group.qty_total_display = str(_format_qty(group.qty_total or 0.0))

    @api.depends("product_image_1920")
    def _compute_product_image_data_uri(self):
        for group in self:
            group.product_image_data_uri = _png_data_uri_from_b64(group.product_image_1920)
    ...
    @api.depends("garment_color_key")
    def _compute_garment_color_hex(self):
        color_map = {
            "black": "#132a99",
            "navy": "#18243a",
            "beige": "#c9b79a",
        ...
        for group in self:
            group.garment_color_hex = color_map.get(group.garment_color_key or "default", "#334155")
    ...
    def _write_size_summary(self, size_map):
        self.ensure_one()
        self.write({
            "size_summary": _format_size_summary(size_map),
            "qty_total": sum(size_map.values()),
        })

    def _marking_has_files_or_notes(self, marking):
        if marking.notes or marking.width_cm or marking.height_cm:
            return True
        return any(
            placement.preview_image or placement.preview_attachment_id or placement.production_attachment_id
            for placement in marking.placement_ids
        )

    def _can_merge_default_markings(self, source_marking):
        self.ensure_one()
        if len(self.marking_ids) != 1 or len(source_marking) != 1:
            return False
        target_marking = self.marking_ids[0]
        source_marking = source_marking[0]
        if self._marking_has_files_or_notes(target_marking) or self._marking_has_files_or_notes(source_marking):
            return False
        return (
            target_marking.technique == source_marking.technique
            and target_marking.position == source_marking.position
            and target_marking.print_color == source_marking.print_color
            and target_marking.size_mode == source_marking.size_mode
        )

    def _merge_marking_sizes(self, target_marking, source_marking):
        size_map = {}
        for size_line in target_marking.size_ids:
            size_map[size_line.size_name] = size_map.get(size_line.size_name, 0.0) + size_line.qty
        for size_line in source_marking.size_ids:
            size_map[size_line.size_name] = size_map.get(size_line.size_name, 0.0) + size_line.qty
        target_marking.size_ids.unlink()
        for size, qty in sorted(size_map.items(), key=_size_sort_key):
            self.env["sp.personalization.size"].create({
                "marking_id": target_marking.id,
                "size_name": size,
                "qty": qty,
            })
        source_marking.unlink()

    def _absorb_duplicate_group(self, duplicate):
        self.ensure_one()
        if duplicate == self:
            return
        size_map = self._get_size_map()
        for size, qty in duplicate._get_size_map().items():
            size_map[size] = size_map.get(size, 0.0) + qty

        vals = {
            "size_summary": _format_size_summary(size_map),
            "qty_total": sum(size_map.values()),
            "original_line_summary": "\n".join(
                item for item in [self.original_line_summary, duplicate.original_line_summary] if item
            ),
            "sale_line_ids": [(6, 0, (self.sale_line_ids | duplicate.sale_line_ids).ids)],
        }
        statuses = {self.status, duplicate.status}
        if "yes" in statuses or self.marking_ids or duplicate.marking_ids:
            vals["status"] = "yes"
        elif statuses == {"no"}:
            vals["status"] = "no"
        else:
            vals["status"] = "pending"
        if not self.product_image_1920 and duplicate.product_image_1920:
            vals["product_image_1920"] = duplicate.product_image_1920
        self.write(vals)

        if duplicate.marking_ids:
            if self._can_merge_default_markings(duplicate.marking_ids):
                self._merge_marking_sizes(self.marking_ids[0], duplicate.marking_ids[0])
            else:
                duplicate.marking_ids.write({"group_id": self.id})
        duplicate.unlink()
...
class SpPersonalizationMarking(models.Model):
    ...
    qty_total_display = fields.Char(compute="_compute_qty_total_display")
    mockup_front_data_uri = fields.Text(compute="_compute_mockup_data_uri")
    mockup_back_data_uri = fields.Text(compute="_compute_mockup_data_uri")
    mockup_front_available = fields.Boolean(compute="_compute_mockup_data_uri")
    mockup_back_available = fields.Boolean(compute="_compute_mockup_data_uri")
    ...
    def _compute_qty_total_display(self):
        for rec in self:
            rec.qty_total_display = str(_format_qty(rec.qty_total or 0.0))

    @api.depends(
        "group_id.product_image_1920",
        "group_id.garment_color_key",
        "print_color_hex",
        "placement_ids.side",
        "placement_ids.placement_key",
        "placement_ids.preview_image",
        "placement_ids.preview_attachment_id.datas",
        "placement_ids.preview_attachment_id.mimetype",
        "placement_ids.production_attachment_id.datas",
        "placement_ids.production_attachment_id.mimetype",
    )
    def _compute_mockup_data_uri(self):
        for rec in self:
            front = rec._compose_mockup_image("front")
            back = rec._compose_mockup_image("back")
            rec.mockup_front_data_uri = front
            rec.mockup_back_data_uri = back
            rec.mockup_front_available = bool(front)
            rec.mockup_back_available = bool(back)

    def _placement_image(self, placement):
        if placement.preview_image:
            image = _image_from_b64(placement.preview_image, max_size=(180, 180))
            if image:
                return image
        attachment = placement.preview_attachment_id if _attachment_is_image(placement.preview_attachment_id) else False
        if not attachment:
            attachment = placement.production_attachment_id if _attachment_is_image(placement.production_attachment_id) else False
        if attachment and attachment.datas:
            return _image_from_b64(attachment.datas, max_size=(180, 180))
        return False

    def _compose_mockup_image(self, side):
        self.ensure_one()
        if not Image or not ImageDraw:
            return ""
        placements = self.placement_ids.filtered(lambda placement: placement.side == side or (side == "front" and placement.side == "side"))
        if side == "back" and not placements:
            return ""
        canvas_size = 360
        canvas = Image.new("RGBA", (canvas_size, canvas_size), (255, 255, 255, 255))
        product = _image_from_b64(self.group_id.product_image_1920, max_size=(310, 310))
        if product:
            canvas.alpha_composite(product, ((canvas_size - product.width) // 2, (canvas_size - product.height) // 2))
        else:
            draw = ImageDraw.Draw(canvas)
            color = _hex_to_rgba(self.group_id.garment_color_hex or "#334155")
            draw.rounded_rectangle((130, 65, 230, 285), radius=28, fill=color)
            draw.rounded_rectangle((88, 105, 145, 180), radius=24, fill=color)
            draw.rounded_rectangle((215, 105, 272, 180), radius=24, fill=color)
            draw.ellipse((150, 50, 210, 95), fill=(255, 255, 255, 255))

        boxes = {
            "front_left": (132, 140, 74, 48),
            "front_center": (128, 145, 92, 56),
            "back": (120, 128, 120, 72),
            "neck": (148, 76, 64, 34),
            "right_sleeve": (230, 142, 58, 48),
            "left_sleeve": (72, 142, 58, 48),
            "cap_front": (130, 134, 100, 62),
            "cap_side": (210, 138, 70, 54),
        }
        border = _hex_to_rgba(self.print_color_hex or "#1f3cff")
        draw = ImageDraw.Draw(canvas)
        for placement in placements:
            left, top, width, height = boxes.get(placement.placement_key, boxes["front_left"])
            ...
            logo = self._placement_image(placement)
            if logo:
                logo.thumbnail((width - 8, height - 8), Image.LANCZOS)
                canvas.alpha_composite(logo, (left + (width - logo.width) // 2, top + (height - logo.height) // 2))
            else:
                draw.text((left + 12, top + max(8, height // 2 - 6)), "LOGO", fill=(20, 20, 20, 255))
        return _png_data_uri_from_image(canvas)
...
class SpPersonalizationPlacement(models.Model):
    ...
    logo_preview_available = fields.Boolean(compute="_compute_logo_preview_available")
    logo_preview_data_uri = fields.Text(compute="_compute_logo_preview_available")
    production_download_url = fields.Char(compute="_compute_download_urls")
    preview_download_url = fields.Char(compute="_compute_download_urls")

    @api.depends(
        "preview_image",
        "preview_attachment_id.datas",
        "preview_attachment_id.mimetype",
        "production_attachment_id.datas",
        "production_attachment_id.mimetype",
    )
    def _compute_logo_preview_available(self):
        for rec in self:
            data_uri = ""
            if rec.preview_image:
                data_uri = _png_data_uri_from_b64(rec.preview_image)
            else:
                attachment = rec.preview_attachment_id if _attachment_is_image(rec.preview_attachment_id) else False
                if not attachment:
                    attachment = rec.production_attachment_id if _attachment_is_image(rec.production_attachment_id) else False
                if attachment and attachment.datas:
                    data_uri = _png_data_uri_from_b64(attachment.datas)
            rec.logo_preview_data_uri = data_uri
            rec.logo_preview_available = bool(data_uri)

    @api.depends("production_attachment_id", "preview_attachment_id")
    def _compute_download_urls(self):
        for rec in self:
            rec.production_download_url = (
                "/web/content/%s?download=true" % rec.production_attachment_id.id
                if rec.production_attachment_id
                else ""
            )
            rec.preview_download_url = (
                "/web/content/%s?download=true" % rec.preview_attachment_id.id
                if rec.preview_attachment_id
                else ""
            )
    ...
    def create_attachment_from_upload(self, upload_file, field_name):
        ...
        if field_name == "preview_attachment_id" or ext in ["png", "jpg", "jpeg", "webp", "svg"]:
            preview_b64 = _preview_b64_from_content(content)
            if preview_b64:
                self.preview_image = preview_b64
        return attachment

    def _download_attachment(self, attachment):
        self.ensure_one()
        if not attachment:
            raise UserError(_("No hay archivo para descargar en esta ubicación."))
        return {
            "type": "ir.actions.act_url",
            "url": "/web/content/%s?download=true" % attachment.id,
            "target": "self",
    ...
    def action_download_production(self):
        self.ensure_one()
        return self._download_attachment(self.production_attachment_id)

    def action_download_preview(self):
        self.ensure_one()
        return self._download_attachment(self.preview_attachment_id)
