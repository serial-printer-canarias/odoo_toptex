...
class SerialPrinterPersonalizationPortal(http.Controller):
    ...
    def _is_ajax_save(self, post):
        return post.get("ajax") == "1" or request.httprequest.headers.get("X-Requested-With") == "XMLHttpRequest"

    def _ajax_response(self, data=None, status=200):
        return request.make_response(
            json.dumps(data or {"ok": True}),
            headers=[("Content-Type", "application/json")]
        )

    def _save_marking_from_request(self, personalization, marking, post):
    ...
    def personalization_form(self, access_token, **post):
        ...
        if request.httprequest.method == "POST":
            ...
            if action == "add_marking":
                ...
                saved = self._save_marking_from_request(personalization, marking, post)
                if self._is_ajax_save(post):
                    return self._ajax_response({"ok": bool(saved), "saved": bool(saved)})
            elif action == "confirm":
                personalization.action_customer_submit(post.get("customer_notes") or personalization.customer_notes)
                if self._is_ajax_save(post):
                    return self._ajax_response({"ok": True, "confirmed": True})
