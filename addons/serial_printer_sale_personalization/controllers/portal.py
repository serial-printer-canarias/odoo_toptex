...
class SerialPrinterPersonalizationPortal(http.Controller):
    ...
    def _process_personalization_post(self, personalization):
        # Guardar siempre primero todo lo visible en pantalla.
    ...
    @http.route(
        ["/personalizacion/<string:access_token>/guardar"],
        type="http",
        auth="public",
        website=True,
        csrf=False,
    ...
    def personalization_save(self, access_token, **post):
        personalization = self._get_personalization(access_token)
        if not personalization:
            return request.not_found()
        self._process_personalization_post(personalization)
