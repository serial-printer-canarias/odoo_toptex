...
class SerialPrinterPersonalizationPortal(http.Controller):
    ...
    def _split_action(self, action):
        action = action or ""
        if ":" in action:
            name, record_id = action.split(":", 1)
            return name, self._to_int(record_id)
        return action, self._to_int(request.params.get("group_id"))
    ...
    def _save_marking_values(self, marking, post, files=None, prefix=None):
        ...
        for placement in marking.placement_ids:
            ...
            if production_file and getattr(production_file, "filename", ""):
                placement.create_attachment_from_upload(production_file, "production_attachment_id")
            if preview_file and getattr(preview_file, "filename", ""):
    ...
    def personalization_form(self, access_token, **post):
        ...
        if request.httprequest.method == "POST":
            action, group_id = self._split_action(post.get("action"))

            # With the portal as a single form, any button first persists all visible data.
            if action in ("save_all", "confirm", "add_marking", "personalize_group", "split_two", "mark_no", "split_group"):
                self._save_all_markings(personalization, post)

            if action == "add_marking":
                group = request.env["sp.personalization.group"].sudo().browse(group_id).exists()
